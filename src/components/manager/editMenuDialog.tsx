...
export function EditMenuDialog({ openDialog, setOpenDialog }: { openDialog: boolean, setOpenDialog: (open: boolean) => void }) {
  return (
    <Dialog open={openDialog}>
      <DialogContent className="sm:max-w-2xl w-full flex flex-col gap-6">
        ...
        <DialogFooter>
            <div className="btn btn-primary text-whereWhite" onClick={() => setOpenDialog(false)}>Save Change</div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
